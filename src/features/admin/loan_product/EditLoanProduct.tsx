...
import { editLoanProduct, getLoanProduct } from './loanProductApi';

type UpdateLoanProductFormInput = {
    name: string,
    interestRate: number,
...
const EditLoanProduct = () => { 
  ...
  const { register, handleSubmit, setValue, formState: { errors } } = useForm<UpdateLoanProductFormInput>({ resolver: yupResolver(validation) });
      ...
      const fetchData = async () => {
        ...
        await getLoanProduct(Number(loanProductId), navigate)?.then(res => {
          if(res.data){
            ...
            setValue("interestRate", productData.interestRate)
            setValue("monitoringFeeRate", productData.monitoringFeeRate)
            setValue("processingFeeRate", productData.processingFeeRate)
            setValue("tenor", productData.tenor)
  ...
  const handleEditLoanProduct = async (form: UpdateLoanProductFormInput) => {
    setIsLoading(true)
    await editLoanProduct(Number(loanProductId), form.name, form.interestRate, form.monitoringFeeRate, form.processingFeeRate, form.tenor, navigate)?.then(res => {
        if(res?.data){
            setIsLoading(false)
  ...
  return (
    <div className="border border-primary rounded-lg m-5 dark:bg-transparent">
      ...
      <form  className="p-6" onSubmit={handleSubmit(handleEditLoanProduct)}> 
        <div className="grid grid-cols-2">
          ...
          <div className='form-group'>
            <label htmlFor="role" className="form-label">Default Tenor</label>
        ...
        <button className='bg-secondary text-white-400 w-60 rounded-full text-lg p-2 mt-4 font-bold' >
            {isLoading ? <CircularProgress size={24} color="inherit" /> : 'Update Loan Product'}
        </button>
      </form>
    </div>
  )
